...
25                        if (nextVal == goal)
26                                return level + 1;
27                        if(nextVal >=0 && nextVal <= 1000 && visited.find(nextVal) == visited.end())
28                          {  q.push(nextVal);
29                            visited[nextVal] = true;}
30                        nextVal = curVal - nums[i];
31                         if (nextVal == goal)
32                                return level + 1;
33                          if(nextVal >=0 && nextVal <= 1000 && visited.find(nextVal) == visited.end())
34                          {  q.push(nextVal);
35                            visited[nextVal] = true;}
36                        nextVal = curVal ^ nums[i];
37                         if (nextVal == goal)
38                                return level + 1;
39                          if(nextVal >=0 && nextVal <= 1000 && visited.find(nextVal) == visited.end())
40                          {  q.push(nextVal);
